...
//   Even row (r, k):  dest = (r+1)*4 +  k    = r*4+k+4  => shift +4, no mask needed
//   Odd  row (r, k):  dest = (r+1)*4 + (k+1) = r*4+k+5  => shift +5
//     k=3 would target col 8 (off-board), so mask RIGHT_EDGE = {7,15,23,31} first.
//
// DOWN-LEFT worked example:
//   Odd  row (r, k):  dest = (r-1)*4 +  k    = r*4+k-4  => shift -4, no mask needed
//   Even row (r, k):  dest = (r-1)*4 + (k-1) = r*4+k-5  => shift -5
//     k=0 would target col -1 (off-board), so mask LEFT_EDGE = {0,8,16,24} first.
//
...
//
// LEFT_EDGE  = {0,8,16,24}  col-0 squares on even rows; no left-diagonal neighbour
// RIGHT_EDGE = {7,15,23,31} col-7 squares on odd rows;  no right-diagonal neighbour
//
