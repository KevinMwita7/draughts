...
// Piece
// Encoding: (color << 1) | kind  →  BLACK/MAN=0, BLACK/KING=1, WHITE/MAN=2,
// WHITE/KING=3 NO_PIECE = 4 (sentinel for empty squares).
...
constexpr Piece make_piece(Color c, PieceKind k) noexcept {
  return Piece((c << 1) | k);
}
...
struct Move {
  ...
  Square to;
  Bitboard
      captured;    // bitboard of every square jumped over (all pieces removed)
  bool promotion;  // true when a man reaches the back rank
  Bitboard captured_kings =
      0;  // filled by do_move: subset of captured that were kings
  uint8_t prev_reversible =
      0;  // filled by do_move: reversible counter before the move
