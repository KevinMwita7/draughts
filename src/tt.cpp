...
static constexpr uint32_t MAX_ENTRIES = 1u << 24; // 16 M entries × 12 B sizeof(TTEntry) = 192 MB
...
void TranspositionTable::resize(uint32_t n_entries) {
    ...
    mask_ = n_entries - 1; // power-of-2: (n-1) is all 1s, so hash & mask_ == hash % n_entries
}
...
bool TranspositionTable::probe(uint64_t hash, TTEntry* out) const {
    ...
    if (e.key == 0 || e.key != static_cast<uint32_t>(hash >> 32))
        return false;
    *out = e;
    return true;
}
...
int TranspositionTable::hashfull() const {
    const auto sample = static_cast<uint32_t>(
        std::min<size_t>(1000, table_.size()));
    int filled = 0;
