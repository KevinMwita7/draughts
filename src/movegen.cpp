...
static void capture_from(Square orig, Square cur,
                          Bitboard enemy, Bitboard empty_bb,
                          Bitboard captured, bool is_king,
                          Color us, MoveList& list)
{
    const Bitboard promo_rank = (us == BLACK) ? BLACK_PROMO_RANK : WHITE_PROMO_RANK;
    const Bitboard c = sq_bb(cur);

    // Man reaching the back rank: crown it and stop — no king continuation this turn
    ...
    auto try_jump = [&](Bitboard mid_bb, Bitboard to_bb) {
        ...
        any = true;
        capture_from(orig, lsb(to_bb),
                     enemy    & ~mid_bb,
                     (empty_bb | c | mid_bb) & ~to_bb,
                     captured | mid_bb,
                     is_king, us, list);
    };
    ...
    if (is_king || us == BLACK) {
        Bitboard mid, to;
        mid = shift_up_right(c); to = shift_up_right(mid); try_jump(mid, to);
        mid = shift_up_left (c); to = shift_up_left (mid); try_jump(mid, to);
    }
    if (is_king || us == WHITE) {
        Bitboard mid, to;
        mid = shift_down_right(c); to = shift_down_right(mid); try_jump(mid, to);
        mid = shift_down_left (c); to = shift_down_left (mid); try_jump(mid, to);
    }

    // Base case: no continuation — emit only if we captured at least one piece
    if (!any && captured)
        list.push({orig, cur, captured, false});
}
...
void draughts::generate_quiets(const Position& pos, MoveList& list) {
    ...
    if (us == BLACK) {
        emit(shift_up_right(men) & empty, true, [](Bitboard b){ return lsb(shift_down_left (b)); });
        emit(shift_up_left (men) & empty, true, [](Bitboard b){ return lsb(shift_down_right(b)); });
    } else {
        emit(shift_down_right(men) & empty, true, [](Bitboard b){ return lsb(shift_up_left  (b)); });
        emit(shift_down_left (men) & empty, true, [](Bitboard b){ return lsb(shift_up_right (b)); });
    }
    // Kings move in all four directions regardless of color; they never promote
    emit(shift_up_right  (kings) & empty, false, [](Bitboard b){ return lsb(shift_down_left (b)); });
    emit(shift_up_left   (kings) & empty, false, [](Bitboard b){ return lsb(shift_down_right(b)); });
    emit(shift_down_right(kings) & empty, false, [](Bitboard b){ return lsb(shift_up_left  (b)); });
    emit(shift_down_left (kings) & empty, false, [](Bitboard b){ return lsb(shift_up_right (b)); });
}
...
bool draughts::has_captures(const Position& pos) {
    ...
    // Apply the shift twice: once to reach the enemy, once to reach the landing square
